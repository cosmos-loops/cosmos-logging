...
namespace Cosmos.Logging.Renders.RendersLib {
    ...
    /// EventIdRenderer
    /// </summary>
    [Renderer("ParentEventId")]
    public class ParentEventIdRenderer : BasicPreferencesRenderer {

        /// <inheritdoc />
        public override string Name => "ParentEventId";

        private static string FixedEventId(ILogEventInfo logEventInfo = null) {
            var ret = logEventInfo?.EventId.Parent?.Id;
            return string.IsNullOrWhiteSpace(ret) ? "no_parent_id" : ret;
        }

...
namespace Cosmos.Logging.Renders.RendersLib {
    /// <summary>
    /// BusinessTraceId renderer
    /// </summary>
    [Renderer("BizTraceId")]
    public class BusinessTraceIdRenderer : BasicPreferencesRenderer {

        /// <inheritdoc />
        public override string Name => "BizTraceId";

        private static string FixedEventId(ILogEventInfo logEventInfo = null) {
            var ret = logEventInfo?.EventId.BusinessTraceId;
            return string.IsNullOrWhiteSpace(ret) ? "no_biz_id" : ret;
        }

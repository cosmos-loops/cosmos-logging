...
namespace Cosmos.Logging.Renders.RendersLib {
    /// <summary>
    /// ScopeTraceId renderer
    /// </summary>
    [Renderer("ScopeTraceId")]
    public class ScopeTraceIdRenderer : BasicPreferencesRenderer {

        /// <inheritdoc />
        public override string Name => "ScopeTraceId";

        private static string FixedEventId(ILogEventInfo logEventInfo = null) {
            var ret = logEventInfo?.EventId.LoggingScopeTraceId;
            return string.IsNullOrWhiteSpace(ret) ? "no_scope_id" : ret;
        }

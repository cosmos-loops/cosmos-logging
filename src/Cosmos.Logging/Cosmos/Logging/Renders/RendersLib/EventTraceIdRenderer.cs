...
namespace Cosmos.Logging.Renders.RendersLib {
    /// <summary>
    /// EventTraceIdRenderer
    /// </summary>
    [Renderer("TraceId")]
    public class EventTraceIdRenderer : BasicPreferencesRenderer {
        /// <inheritdoc />
        public override string Name => "TraceId";

        private static string FixedEventId(ILogEventInfo logEventInfo = null) {
            var ret = logEventInfo?.EventId.TraceId;
            return string.IsNullOrWhiteSpace(ret) ? "no_trace_id" : ret;
        }

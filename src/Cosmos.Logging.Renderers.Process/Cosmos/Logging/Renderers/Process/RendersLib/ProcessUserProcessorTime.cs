...
namespace Cosmos.Logging.Renderers.Process.RendersLib {
    /// <summary>
    /// UserProcessorTime renderer
    /// </summary>
    [Renderer("UserProcessorTime")]
    public class ProcessUserProcessorTime : BasicPreferencesRenderer {

        /// <inheritdoc />
        public override string Name => "UserProcessorTime";

        private static string GetUserProcessorTime(string paramsText) {
            var ts = System.Diagnostics.Process.GetCurrentProcess().UserProcessorTime;
...
//                    return $"{d} {(d > 1 ? "days" : "day")} {h} {(d > 1 ? "Hours" : "Hour")} {m} {(m > 1 ? "Minutes" : "Minute")} {s}.{ms} {(s > 1 ? "Seconds" : "Second")}";
        ...
        public override string ToString(string format, string paramsText, ILogEventInfo logEventInfo = null, IFormatProvider formatProvider = null) {
            return GetUserProcessorTime(paramsText);
        }
        ...
        public override string ToString(IList<Func<object, IFormatProvider, object>> formattingFuncs, string paramsText,
            ILogEventInfo logEventInfo = null, IFormatProvider formatProvider = null) {
            return formattingFuncs.ToFormat(GetUserProcessorTime(paramsText), formatProvider);
        }

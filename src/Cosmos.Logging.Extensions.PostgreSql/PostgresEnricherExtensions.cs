...
namespace Cosmos.Logging
{
    ...
    {
        public static DatabaseIntegration UsePostgreSql(this DatabaseIntegration integration, Action<PostgresEnricherOptions> settingAct = null,
        ...
        private static DatabaseIntegration UsePostgreSqlCore(DatabaseIntegration integration, IOptions<PostgresEnricherOptions> settings,
            Action<IConfiguration, PostgresEnricherConfiguration, LoggingConfiguration> config = null)
        {
            ...
            if (serviceImpl != null)
            {
                serviceImpl.AddExtraSinkSettings<PostgresEnricherOptions, PostgresEnricherConfiguration>(settings.Value, (conf, sink, configuration) => config?.Invoke(conf, sink, configuration));
                serviceImpl.AddDependency(s => s.TryAdd(ServiceDescriptor.Singleton(settings)));
                serviceImpl.AddDependency(s => s.TryAdd(ServiceDescriptor.Singleton(typeof(NpgsqlLogActivation), provider => new NpgsqlLogActivation(provider.GetService<ILoggingServiceProvider>(), settings))));
